...
git clone https://github.com/blastdev/blast-core
mkdir -p release
cd blast-core/depends
...
sudo find ${DISTNAME}/bin -type f -executable -exec ../blast-core/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../blast-core/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo blast-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find blast-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/blast-core
...
cp -f blast-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
sudo rm -rf win64
cp -rf blast-core/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir unsigned
mv ~/blast-core/blast-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/blast-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer blast-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer blast-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
cd ~/
sudo rm -rf win32
cp -rf blast-core/contrib/windeploy ~/sign/win32
...
#transfer blast-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/blast-core/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/blastdev/blast-core
mkdir ~/blast-core/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder blast-core/depends/SDKs
cd blast-core/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/blast-core
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Blast-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/blast-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer blast-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
