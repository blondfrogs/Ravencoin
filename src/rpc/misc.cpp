...
#ifdef ENABLE_WALLET
        // Case 1: BLAST address and we have full public key:
    ...
    if (request.fHelp || request.params.size() != 2) {
        ...
        if(sporkManager.UpdateSpork(nSporkID, nValue, *g_connman)){
            ...
            throw std::runtime_error(
                ...
                "\nExamples:\n"
                + HelpExampleCli("spork", "SPORK_3_MASTERNODE_PAYMENT_ENFORCEMENT 1577836800")
                + HelpExampleRpc("spork", "\"SPORK_3_MASTERNODE_PAYMENT_ENFORCEMENT\", 1577836800"));
        }
    }
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "blast-cli and the GUI. There is no server-side difference."
        );
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    ...
    if (includeAssets) {
        ...
        if (!mempool.getAddressIndex(addresses, BLAST, indexes)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
        }
    }
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    ...
    if (includeAssets) {
        ...
        for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
            if (!GetAddressIndex((*it).first, (*it).second, BLAST, addressIndex)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
            }
        }
...
UniValue getaddresstxids(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
        if (includeAssets) {
            ...
            if (start > 0 && end > 0) {
                if (!GetAddressIndex((*it).first, (*it).second, BLAST, addressIndex, start, end)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
                }
            } else {
                if (!GetAddressIndex((*it).first, (*it).second, BLAST, addressIndex)) {
