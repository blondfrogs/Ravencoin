...
#include "algo/ethhash/attributes.h"
#include "algo/ethhash/bit_manipulation.h"
#include "algo/ethhash/endianness.hpp"
#include "algo/ethhash/primes.h"
...
extern "C" {
    #include "algo/siphash/siphash.h"
...
inline hash512 hash_final_512(const hash512& seed, const hash512& mix_hash)
{
    uint8_t final_data[sizeof(seed) + sizeof(mix_hash)];
    std::memcpy(&final_data[0], seed.bytes, sizeof(seed));
    std::memcpy(&final_data[sizeof(seed)], mix_hash.bytes, sizeof(mix_hash));
    return keccak512(final_data, sizeof(final_data));
}
