...
{
    QFontDatabase::addApplicationFont(":/fonts/exo2-bold");
    QFontDatabase::addApplicationFont(":/fonts/exo2-bolditalic");
    QFontDatabase::addApplicationFont(":/fonts/exo2-extrabold");
    QFontDatabase::addApplicationFont(":/fonts/exo2-extrabolditalic");
    QFontDatabase::addApplicationFont(":/fonts/exo2-italic");
    QFontDatabase::addApplicationFont(":/fonts/exo2-light");
    QFontDatabase::addApplicationFont(":/fonts/exo2-lightitalic");
    QFontDatabase::addApplicationFont(":/fonts/exo2-regular");
    QFontDatabase::addApplicationFont(":/fonts/exo2-semibold");
    QFontDatabase::addApplicationFont(":/fonts/exo2-semibolditalic");
...
void BitcoinGUI::createActions()
{
    ...
    QFont fontAlt = QFont();
    fontAlt.setPixelSize(22);
    fontAlt.setLetterSpacing(QFont::SpacingType::AbsoluteSpacing, -0.43);
#if !defined(Q_OS_MAC)
    font.setFamily("Open Sans");
    fontAlt.setFamily("Exo2");
#endif
    font.setWeight(QFont::Weight::ExtraLight);
    fontAlt.setWeight(QFont::Weight::ExtraLight);
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and blast: URIs)"));
    receiveCoinsAction->setToolTip(receiveCoinsAction->statusTip());
    receiveCoinsAction->setCheckable(true);
    receiveCoinsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_3));
    receiveCoinsAction->setFont(fontAlt);
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to BLAST addresses"));
    ...
    createAssetAction->setFont(fontAlt);
    ...
    manageAssetAction->setFont(fontAlt);
    ...
    messagingAction->setFont(fontAlt);
    ...
    if (!fLiteMode && settings.value("fShowMasternodesTab").toBool()) {
        ...
        masternodeAction->setFont(fontAlt);
#ifdef Q_OS_MAC
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BLAST addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BLAST addresses"));
    ...
    openAction->setStatusTip(tr("Open a blast: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BLAST command-line options").arg(tr(PACKAGE_NAME)));
...
//        toolbar->addAction(votingAction);
...
#if !defined(Q_OS_MAC)
        stringToUse = exo2FontString; // stringToUse = openSansFontString;
#else
        ...
        QVBoxLayout* bitcoinLabelLayout = new QVBoxLayout(toolbarWidget);
        bitcoinLabelLayout->addWidget(label);
        bitcoinLabelLayout->addWidget(toolbar);
        bitcoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        bitcoinLabelLayout->addStretch(1);
        ...
        currentMarketFont.setFamily("Open Sans"); // currentMarketFont.setFamily("Exo2");
        ...
        QLabel* labelBtcBlast = new QLabel();
        labelBtcBlast->setText("BTC / BLAST");
        labelBtcBlast->setContentsMargins(15,0,0,0);
        labelBtcBlast->setFixedHeight(75);
        labelBtcBlast->setAlignment(Qt::AlignVCenter);
        labelBtcBlast->setStyleSheet(STRING_LABEL_COLOR);
        labelBtcBlast->setFont(currentMarketFont);
        ...
        priceLayout->addWidget(labelBtcBlast, 0 , Qt::AlignVCenter | Qt::AlignLeft);
        ...
        QObject::connect(networkManager, &QNetworkAccessManager::finished,
                    ...
                    QRegExp rx("\\d*(\\.\\d*)?[eE]-\\d*|\\d*.\\d\\d\\d\\d\\d\\d\\d\\d");
                    rx.indexIn(answer);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to BLAST network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("BLAST"); // default title
...
void BitcoinGUI::checkAssets()
{
    ...
    if(AreAssetsDeployed()) {
        ...
        transferAssetAction->setToolTip(tr("Transfer assets to BLAST addresses"));
...
{
    request->setUrl(QUrl("https://api.coingecko.com/api/v3/simple/price?ids=blast&vs_currencies=btc"));
