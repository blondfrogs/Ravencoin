...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BLAST address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no blast: URI
    if(!uri.isValid() || uri.scheme() != QString("blast"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    //
    //    Cannot handle this later, because blast:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("blast://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "blast:");
    }
...
{
    QString ret = QString("blast:%1").arg(info.address);
    int paramCount = 0;
...
bool openBitcoinConf()
{
    ...
    /* Open blast.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BLAST.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BLAST (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BLAST (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "blast.desktop";
    return GetAutostartDir() / strprintf("blast-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a blast.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BLAST\n";
        else
            optionFile << strprintf("Name=BLAST (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the blast app
...
bool GetStartOnSystemStartup()
{
    CFURLRef blastAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (blastAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, blastAppUrl);

    CFRelease(blastAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add blast app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, blastAppUrl, nullptr, nullptr);
    }
