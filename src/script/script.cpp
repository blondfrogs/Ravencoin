...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** BLAST START */
    case OP_BLAST_ASSET              : return "OP_BLAST_ASSET";
    /** BLAST END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_BLAST_ASSET) { // OP_BLAST_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == BLAST_R) { // Check to see if BLAST starts at 27 ( this->size() < 105)
                if ((*this)[28] == BLAST_V)
                    if ((*this)[29] == BLAST_N)
                        index = 30;
            } else {
                if ((*this)[28] == BLAST_R) // Check to see if BLAST starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == BLAST_V)
                        if ((*this)[30] == BLAST_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == BLAST_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == BLAST_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == BLAST_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == BLAST_R) {
