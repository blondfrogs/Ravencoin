...
Currently only `Source10` is used, for the example `blast.conf` file.
...
in the bitcoin git repository. At present only an SVG version of the BLAST
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the BLAST
...
idea to build BLAST Core against a static version of that library compiled
...
BLAST-Core against OpenSSL or use a patched version of Boost in the build
system.
