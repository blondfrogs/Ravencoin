...
"""BLAST P2P network half-a-node.

This python code was modified from ArtForz' public domain  half-a-node, as
found in the mini-node branch of http://github.com/jgarzik/pynode.

NodeConn: an object which manages p2p connectivity to a BLAST node
...
COIN = 100000000 # 1 blast in satoshis
...
x16r_hash_cmd = os.path.dirname(os.path.realpath(__file__)) + "/../../../src/test/test_blast_hash"
def hash_x16r(s):
    cmd = [x16r_hash_cmd, s]
...
# Objects that map to blastd objects, which can be serialized/deserialized
...
class CTransaction():
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in blastd
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_headers():
    ...
    def deserialize(self, f):
        # comment in blastd indicates these should be deserialized as blocks
...
class NodeConnCB():
    """Callback and helper functions for P2P connection to a blastd node.
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to BLAST Node: %s:%d' % (self.dstaddr, self.dstport))
