...
def main():
    ...
    tmpdir = "%s/blast_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_blastd = config["components"].getboolean("ENABLE_BLASTD")
    ...
    if not (enable_wallet and enable_cli and enable_blastd):
        print("No functional tests to run. Wallet, utils, and blastd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if blastd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "blastd"]) is not None:
            print("%sWARNING!%s There is already a blastd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "BLASTD" not in os.environ:
        os.environ["BLASTD"] = build_dir + '/src/blastd' + exeext
        os.environ["BLASTCLI"] = build_dir + '/src/blast-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `blast-cli help` (`rpc_interface.txt`).
